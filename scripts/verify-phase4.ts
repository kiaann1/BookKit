/**
 * Verify Phase 4 exit criteria locally or against a deployed URL.
 *
 * Usage:
 *   npm run verify:phase4
 *   npm run test:recommendations
 *   npm run verify:phase4 -- --url https://book-kit-psi.vercel.app
 */
...
async function verifyLocal() {
  const { runPhase4Checks } = await import("../src/lib/health/phase4");
  return runPhase4Checks();
}

async function verifyRemote(baseUrl: string) {
  const url = new URL("/api/health/phase4", baseUrl).toString();
...
function printReport(
  ...
  console.log(
    `\n${report.ok ? "Phase 4 complete." : "Phase 4 incomplete — fix the items above."}\n`,
  );
}
